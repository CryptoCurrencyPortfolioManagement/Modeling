...
class sumW2VRecurrentPriceClassifier(nn.Module):
    def __init__(self, use_price, number_assets= 10, embed_dim=301, use_attention= False, rnn_layers = 1, linear_layers= 1, dropout_prop= 0.5):
...
class ModelWrapper(BaseEstimator, ClassifierMixin):
    def __init__(self, batch_size, ccs, use_price= True, epochs= 2, learning_rate= 1e-5, use_attention= False, rnn_layers= 1, linear_layers= 1, dropout_prop= 0.5, device= "cpu"):
        self.device = device
        self.model = sumW2VRecurrentPriceClassifier(use_price, embed_dim= 300 + len(ccs), use_attention= use_attention, rnn_layers= rnn_layers, linear_layers= linear_layers, dropout_prop= dropout_prop)
...
if __name__ == "__main__":
    with wandb.init() as run:
        ...
        param = wandb.config
        #param = {"source": "CoinTelegraph", "window_size": 1, "learning_rate": 0.0002651, "use_attention": False, "rnn_layers": 3, "linear_layers": 3, "use_price": True, "dropout": 0.5, "epochs": 2}
        ...
        for i, (test_roll_start_date, test_roll_end_date) in enumerate(windows):
            ...
            my_model = ModelWrapper(batch_size= 64, ccs= test_env.labels, learning_rate=param["learning_rate"], use_attention= param["use_attention"], rnn_layers= param["rnn_layers"], linear_layers= param["linear_layers"], use_price=param["use_price"], dropout_prop= param["dropout"], epochs= param["epochs"], device= device)
