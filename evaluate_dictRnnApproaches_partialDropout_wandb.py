...
class DictRecurrentPriceClassifier(nn.Module):
    def __init__(self, use_price, num_assets= 10, embed_dim=301, use_attention= False, rnn_layers = 1, linear_layers= 1, dropout_prop= 0.5):
...
class ModelWrapper(BaseEstimator, ClassifierMixin):
    def __init__(self, batch_size, ccs, use_price= True, epochs= 2, learning_rate= 1e-5, use_attention= False, rnn_layers= 1, linear_layers= 1, input_dim= 1, dropout_prop= 0.5):
        ...
        if len(GPUtil.getAvailable(order='memory', limit=1, maxLoad=0.5, maxMemory=0.5)) > 0:
            ...
            self.device = "cpu"
        self.model = DictRecurrentPriceClassifier(use_price, embed_dim= input_dim + len(ccs), use_attention= use_attention, rnn_layers= rnn_layers, linear_layers= linear_layers, dropout_prop= dropout_prop)
...
if __name__ == "__main__":
    with wandb.init() as run:
        param = wandb.config
        # param = {"window_size": 1, "source": "CoinTelegraph", "use_price": True, "learning_rate": 0.000947, "use_attention": True, "rnn_layers": 5, "linear_layers": 3, "dicts": "bert+LM+Vader", "dropout": 0.0, "epochs": 3}
        ...
        for i, (test_roll_start_date, test_roll_end_date) in enumerate(windows):
            ...
            my_model = ModelWrapper(batch_size= 64, ccs= test_env.labels, learning_rate=param["learning_rate"], use_attention= param["use_attention"], rnn_layers= param["rnn_layers"], linear_layers= param["linear_layers"], use_price=param["use_price"], input_dim= input_dim, dropout_prop= param["dropout"], epochs= param["epochs"])
