...
class Dropout_partial_binary(nn.Module):
    ...
    def forward(self, X):
        ...
        old_shape = X_to_modify.shape
        X_to_modify = X_to_modify.reshape((-1, len(self.include_index)))
        ...
        X_to_modify = X_to_modify.reshape(old_shape)
...
# model to combine w2v embeddings with mutliple gru layers
class W2VRecurrentPriceClassifier(nn.Module):
    def __init__(self, use_price, num_assets, embed_dim=300, portfolio_dim= 32,  use_attention= True, rnn_layers= 1, linear_layers= 1, portfolio_layers= 1, dropout_prop= 0.5):
        super(W2VRecurrentPriceClassifier, self).__init__()
        ...
        self.embed_words_gru = nn.GRU(embed_dim, embed_dim, self.rnn_layers, batch_first= True)
        self.embed_texts_gru = nn.GRU(embed_dim + num_assets, embed_dim, self.rnn_layers, batch_first= True)
        ...
        if self.use_attention:
            self.embed_words_attention = Attention(embed_dim, embed_dim, embed_dim, batch_first= True)
            self.embed_texts_attention = Attention(embed_dim, embed_dim, embed_dim, batch_first= True)
        ...
        self.portfolio.append(nn.Linear(self.num_assets, portfolio_dim))
        for i in range(self.portfolio_layers - 1):
            self.portfolio.append(nn.Linear(portfolio_dim, portfolio_dim))
        self.portfolio.append(nn.Linear(portfolio_dim, self.num_assets))
    ...
    def forward(self, text, price= None):
        ...
        for lag_ind in range(text.shape[1]):
            msg_temp = []
            for msg_ind in range(text.shape[2]):
                outputs, hidden = self.embed_words_gru(text[:, lag_ind, msg_ind, :, 1:])
                if isinstance(hidden, tuple):
                    hidden = hidden[1]
                hidden = hidden[-1]
                if self.use_attention:
                    energy, hidden = self.embed_words_attention(hidden, outputs, outputs)
                cc_temp = torch.max(text[:, lag_ind, msg_ind, :, 0], dim=1).values.to(dtype=torch.long)
                cc_temp = torch.eye(self.num_assets)[cc_temp].to(hidden.device)
                hidden = torch.cat((cc_temp, hidden), dim= 1)
                msg_temp.append(hidden)
            msg_temp = torch.stack(msg_temp, dim=1)
            outputs, hidden = self.embed_texts_gru(msg_temp)
            if isinstance(hidden, tuple):
                hidden = hidden[1]
            hidden = hidden[-1]
            if self.use_attention:
                energy, hidden = self.embed_texts_attention(hidden, outputs, outputs)
            lag_temp.append(hidden)
...
class ModelWrapper(BaseEstimator, ClassifierMixin):
    def __init__(self, batch_size, ccs, use_price= True, epochs= 2, learning_rate= 1e-5, use_attention= False, rnn_layers= 1, linear_layers= 1, portfolio_layers= 1, portfolio_dim=32, dropout_prop= 0.5):
        ...
        if len(GPUtil.getAvailable(order='memory', limit=1, maxLoad=0.5, maxMemory=0.5)) > 0:
            ...
            self.device = "cpu"
        #self.device = "cuda"
        self.model = W2VRecurrentPriceClassifier(use_price, num_assets= len(ccs), embed_dim= 299 + len(ccs), use_attention= use_attention, rnn_layers= rnn_layers, linear_layers= linear_layers, portfolio_layers= portfolio_layers, portfolio_dim= portfolio_dim, dropout_prop= dropout_prop)
        ...
        for i, cc in enumerate(self.ccs):
            ...
            for j in range(X_text.shape[3]):
                ...
                for l in range(X_text.shape[2]):
                    X_text_new_temp2.append(np.apply_along_axis(self._create_w2v_input, 1, X_text[:, i, l, j, :], cc))
        ...
        for epoch in range(self.epochs):
            ...
            if self.use_price:
                for X_text_batch, X_price_batch, y_batch in tqdm(dataloader, desc="Epoch: {}".format(epoch + 1)):
                    X_text_batch = X_text_batch.numpy()
                    X_text_batch_shape = X_text_batch.shape
                    X_text_batch = X_text_batch.reshape((-1, X_text_batch_shape[-1]))
                    X_text_batch = np.apply_along_axis(self._create_w2v_vector, 1, X_text_batch)
                    X_text_batch = X_text_batch.reshape((*X_text_batch_shape[:-1], -1))
                    X_text_batch = torch.tensor(X_text_batch)
    ...
    def predict(self, X):
        ...
        for i, cc in enumerate(self.ccs):
            ...
            for j in range(X_text.shape[3]):
                X_text_new_temp2 = []
                for l in range(X_text.shape[2]):
                    X_text_new_temp2.append(np.apply_along_axis(self._create_w2v_input_complete, 1, X_text[:, i, l, j, :], cc))
                X_text_new_temp1.append(np.stack(X_text_new_temp2, axis=1))
            X_text_new.append(np.stack(X_text_new_temp1, axis=1))
        X_text_new = np.stack(X_text_new, axis=1)
    ...
    def _create_w2v_input(self, data, cc):
        ...
        cc_data = np.empty(data.shape)
        cc_data[:] = np.nan
        if np.sum((np.isnan(data)).astype(int)) > 0:
            nan_idx = np.min(np.argwhere(np.isnan(data)))
            data_pre = np.empty(len(data) - nan_idx)
            data_pre[:] = np.nan
            data = np.concatenate((data_pre, data[:nan_idx]))
            cc_data[len(data) - nan_idx:] = cc_ind
        else:
            cc_data[:] = cc_ind
        return np.stack((cc_data, data), axis=1)

    def _create_w2v_vector(self, index):
        cc_ind = int(index[0])
        cc_array = np.zeros(len(self.ccs))
        cc_array[cc_ind] = 1
        w2v_array = self._create_w2v_vector_complete(index[1])
        return np.concatenate((cc_array, w2v_array))

    def _create_w2v_input_complete(self, data, cc):
        if np.sum((np.isnan(data)).astype(int)) > 0:
            nan_idx = np.min(np.argwhere(np.isnan(data)))
            data_pre = np.empty(len(data) - nan_idx)
            data_pre[:] = np.nan
        ...
        for index in data:
            if np.isnan(index):
                temp_w2v = np.empty(300+len(self.ccs))
                temp_w2v[:] = np.nan
            else:
                temp_w2v = self._create_w2v_vector_complete(index)
                temp_w2v = np.concatenate((cc_array, temp_w2v))
            w2v_array.append(temp_w2v)
        return np.stack(w2v_array, axis=0)

    def _create_w2v_vector_complete(self, index):
        if np.isnan(index):
            out = np.empty(300)
            out[:] = np.nan
        else:
            out = fasttext_model[int(index)]
        return out
...
if __name__ == "__main__":
    with wandb.init() as run:
        param = wandb.config
        #param = {"learning_rate": 0.00018217168033522052, "linear_layers": 2, "rnn_layers": 3, "source": "CoinTelegraph", "use_attention": False, "use_price": False, "window_size": 1, "dropout": 0.5, "epochs": 2, "portfolio_layers": 1, "portfolio_dim": 32}

        # load Data
        text_df = pq.ParquetDataset("data/Content.parquet", validate_schema=False, filters=[('cc', 'in', ccs), ('source', '=', param["source"])])
        text_df = text_df.read(columns=["content_w2vToken"]).to_pandas()
        ...
        for i, (test_roll_start_date, test_roll_end_date) in enumerate(windows):
            ...
            #test_env.render_all(path= "./plots/{}_CV".format("_".join([str(x).replace("/","-") for x in list(dict(param).values())])) + "{}.png".format(i))
