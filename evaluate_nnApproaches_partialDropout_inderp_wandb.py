...
class PortfolioClassifier(nn.Module):
    def __init__(self, number_assets= 10, portfolio_dim= 32, portfolio_layers= 1):
        super(PortfolioClassifier, self).__init__()
...
class ModelWrapper(BaseEstimator, ClassifierMixin):
    def __init__(self, batch_size, ccs, bert_model, epochs= 2, learning_rate= 1e-6, freeze_bert= False, dropout_prop= 0.5, portfolio_dim= 32, portfolio_layers= 1):
        ...
        self.portfolio_model = PortfolioClassifier(len(ccs), portfolio_dim= portfolio_dim, portfolio_layers= portfolio_layers)
        ...
        self.portfolio_model.to(self.device)
    ...
    def fit(self, X, y):
        ...
        dataloader = DataLoader(CustomDataset(X1=X_text_new, y=y_new), batch_size=self.batch_size, shuffle=False, drop_last= True)
        ...
        for epoch in range(self.epochs):
            ...
            for X_text_batch, y_batch in tqdm(dataloader, desc="Epoch: {}".format(epoch + 1)):
                ...
                orig_shape = X_text_batch.shape[:2]
                X_text_batch = torch.reshape(X_text_batch, (-1, *X_text_batch.shape[2:]))
                preds_batch = self.portfolio_model(self.model(input_ids=X_text_batch[:, 0, :],
                                         token_type_ids=X_text_batch[:, 1, :],
                                         attention_mask=X_text_batch[:, 2, :]))
                preds_batch = torch.reshape(preds_batch, (orig_shape[0], -1))
...
if __name__ == "__main__":
    with wandb.init() as run:
        param = wandb.config
        #param = {"freeze_bert": False, "learning_rate": 0.0008, "model": "yiyanghkust/finbert-tone", "source": "CoinTelegraph", "window_size": 1, "portfolio_dim": 32, "portfolio_layers": 1, "dropout": 0.5, "epochs": 2}
        ...
        for i, (test_roll_start_date, test_roll_end_date) in enumerate(windows):
            ...
            my_model = ModelWrapper(batch_size= 128, ccs= test_env.labels, bert_model= param["model"], learning_rate= param["learning_rate"], freeze_bert= param["freeze_bert"], dropout_prop= param["dropout"], epochs= param["epochs"], portfolio_dim= param["portfolio_dim"], portfolio_layers= param["portfolio_layers"])
