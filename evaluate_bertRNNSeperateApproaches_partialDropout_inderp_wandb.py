...
# create model to combine hidden states via gru layers (bert model for hidden states is not part of this model => will not be trained)
class RecurrentPriceClassifier(nn.Module):
    def __init__(self, use_price, number_assets= 10, embed_dim=300, embed_model_dim= 768, portfolio_dim= 32, use_attention= False, rnn_layers= 1, linear_layers= 1, portfolio_layers= 1):
        super(RecurrentPriceClassifier, self).__init__()
        ...
        self.embed_gru = nn.GRU(embed_model_dim, embed_dim + 1, self.rnn_layers, batch_first= True)

        if self.use_price:
            self.gru = nn.GRU(embed_dim + 2, embed_dim, self.rnn_layers, bidirectional= False, batch_first= True)
        else:
            self.gru = nn.GRU(embed_dim + 1, embed_dim, self.rnn_layers, bidirectional= False, batch_first= True)

        if self.use_attention:
            self.embed_attention = Attention(embed_dim + 1, embed_dim + 1, embed_dim + 1, batch_first= True)
    ...
    def forward(self, text, price= None):
        ...
        for lag_ind in range(text.shape[1]):
            outputs, hidden = self.embed_gru(text[:, lag_ind, :, :])
...
class ModelWrapper(BaseEstimator, ClassifierMixin):
    def __init__(self, batch_size, ccs, bert_model, use_price= True, epochs= 2, learning_rate= 1e-5, use_attention= False, rnn_layers= 1, linear_layers= 1, portfolio_layers= 1, portfolio_dim=32, dropout_prop= 0.5):
        ...
        self.model = RecurrentPriceClassifier(use_price, number_assets= len(ccs), use_attention= use_attention, rnn_layers= rnn_layers, linear_layers= linear_layers, portfolio_layers= portfolio_layers, portfolio_dim= portfolio_dim)
        ...
        for epoch in range(self.epochs):
            ...
            if self.use_price:
                for X_text_batch, X_price_batch, y_batch in tqdm(dataloader, desc="Epoch: {}".format(epoch + 1)):
                    ...
                    with torch.no_grad():
                        lag_temp = []
                        for lag_ind in range(X_text_batch.shape[2]):
                            msg_temp = X_text_batch[:, :, lag_ind, :, :]
                            old_shape = msg_temp.shape
                            msg_temp = torch.reshape(msg_temp, (-1, *msg_temp.shape[3:]))
                            msg_temp = self.bert_model(
                                input_ids=msg_temp[:, 0, :],
                                token_type_ids=msg_temp[:, 1, :],
                                attention_mask=msg_temp[:, 2, :]
                            )
                            msg_temp = msg_temp["last_hidden_state"][:, 0, :]
                            msg_temp = torch.reshape(msg_temp, (*old_shape[:3], -1))
                            lag_temp.append(msg_temp)
                        X_text_batch = torch.stack(lag_temp, dim=2)
    ...
    def predict(self, X):
        ...
        with torch.no_grad():
            lag_temp = []
            for lag_ind in range(X_text_new.shape[2]):
                msg_temp = X_text_new[:, :, lag_ind, :, :]
                old_shape = msg_temp.shape
                msg_temp = torch.reshape(msg_temp, (-1, *msg_temp.shape[3:]))
                msg_temp = self.bert_model(
                    input_ids=msg_temp[:, 0, :],
                ...
                msg_temp = torch.reshape(msg_temp, (*old_shape[:3], -1))
                lag_temp.append(msg_temp)
            X_text_new = torch.stack(lag_temp, dim=2)

            if self.use_price:
                X_price = X_price.to(self.device, dtype=torch.float)
                preds = self.model(X_text_new, X_price)
                scores = torch.tanh(preds).detach().cpu().numpy()
            else:
                preds = self.model(X_text_new, X_price)
                scores = torch.tanh(preds).detach().cpu().numpy()
...
if __name__ == "__main__":
    with wandb.init() as run:
        param = wandb.config
        #param = {"dropout": 0.5573, "learning_rate": 0.000318, "linear_layers": 3, "model": "bert-base-uncased", "rnn_layers": 2, "source": "Twitter", "use_attention": False, "use_price": False, "window_size": 2, "epochs": 1, "portfolio_dim": 64, "portfolio_layers": 5, "portfolio_learning_rate": 0.0001453}  # load Data
        ...
        for i, (test_roll_start_date, test_roll_end_date) in enumerate(windows):
            ...
            my_model = ModelWrapper(batch_size=10, ccs=test_env.labels, bert_model=param["model"], use_price= param["use_price"], learning_rate=param["learning_rate"],use_attention=param["use_attention"], rnn_layers=param["rnn_layers"],linear_layers=param["linear_layers"], portfolio_layers= param["portfolio_layers"], portfolio_dim= param["portfolio_dim"], dropout_prop= param["dropout"], epochs= param["epochs"])
