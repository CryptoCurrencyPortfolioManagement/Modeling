...
class TransformerStackPriceClassifier(nn.Module):
    def __init__(self, use_price, n_tokens, number_assets= 10, n_head= 12, transformer_layers= 1, embed_dim=300, portfolio_dim= 32, use_attention= False, linear_layers= 1, portfolio_layers= 1, dropout= 0.1):
    ...
    def forward(self, text, price= None):
        ...
        combined = torch.reshape(combined, (orig_shape[0], -1))
...
class ModelWrapper(BaseEstimator, ClassifierMixin):
    def __init__(self, batch_size, ccs, n_head, transformer_layers, use_price= True, epochs= 2, learning_rate= 1e-5, use_attention= False, linear_layers= 1, portfolio_layers= 1, portfolio_dim=32, dropout_prop= 0.5):
        ...
        self.model = TransformerStackPriceClassifier(use_price, len(self.tokenizer), number_assets= len(ccs), n_head= n_head, transformer_layers= transformer_layers, use_attention= use_attention, linear_layers= linear_layers, portfolio_layers= portfolio_layers, portfolio_dim= portfolio_dim)
...
if __name__ == "__main__":
    with wandb.init() as run:
        param = wandb.config
        #param = {"source": "CoinTelegraph", "window_size": 2, "learning_rate": 0.00001, "use_attention": True, "linear_layers": 1, "use_price": True, "n_head": 12, "transformer_layers": 1, "dropout": 0.5, "portfolio_layers": 1, "portfolio_dim": 32, "epochs": 2}
        ...
        for i, (test_roll_start_date, test_roll_end_date) in enumerate(windows):
            ...
            my_model = ModelWrapper(epochs= param["epochs"], batch_size= 2, ccs= test_env.labels, n_head = param["n_head"], transformer_layers = param["transformer_layers"], learning_rate=param["learning_rate"], use_attention= param["use_attention"], linear_layers= param["linear_layers"], portfolio_layers= param["portfolio_layers"], portfolio_dim= param["portfolio_dim"], use_price= param["use_price"], dropout_prop= param["dropout"])
