...
# create model to combine dictionary data over time via grus
class DictRecurrentPriceClassifier(nn.Module):
    def __init__(self, use_price, num_assets= 10, embed_dim=301, portfolio_dim= 32, use_attention= False, rnn_layers = 1, linear_layers= 1, portfolio_layers= 1, dropout_prop= 0.5):
        super(DictRecurrentPriceClassifier, self).__init__()
        ...
        for i in range(self.portfolio_layers - 1):
            self.portfot.append(nn.Linear(portfolio_dim, portfolio_dim))
...
class ModelWrapper(BaseEstimator, ClassifierMixin):
    def __init__(self, batch_size, ccs, use_price= True, epochs= 2, learning_rate= 1e-5, use_attention= False, rnn_layers= 1, linear_layers= 1, portfolio_layers= 1, portfolio_dim=32, input_dim= 1, dropout_prop= 0.5):
        ...
        if len(GPUtil.getAvailable(order='memory', limit=1, maxLoad=0.5, maxMemory=0.5)) > 0:
            ...
            self.device = "cpu"
        self.model = DictRecurrentPriceClassifier(use_price, embed_dim= input_dim + len(ccs), use_attention= use_attention, rnn_layers= rnn_layers, linear_layers= linear_layers, portfolio_layers= portfolio_layers, portfolio_dim= portfolio_dim, dropout_prop= dropout_prop)
    ...
    def fit(self, X, y):
        ...
        for i, cc in enumerate(self.ccs):
            ...
            for j in range(X_text.shape[2]):
                X_text_new_temp.append(np.apply_along_axis(self._create_input, 1, X_text[:, i, j, :], cc))
    ...
    def _create_input(self, data, cc):
...
class NN_agent:
    ...
    def choose_action(self, obs):
        ...
        X_temp_text = np.moveaxis(X_temp_text, 0, 3)
...
if __name__ == "__main__":
    with wandb.init() as run:
        param = wandb.config
        #param = {"window_size": 3, "source": "CoinTelegraph", "use_price": False, "learning_rate": 0.000947, "use_attention": False, "rnn_layers": 2, "linear_layers": 3, "dicts": "Vader", "dropout": 0.0, "epochs": 3, "portfolio_layers": 1, "portfolio_dim": 32}

        columns = []
        if "bert" in param["dicts"]:
            columns.append("sentiment_bert")
        if "LM" in param["dicts"]:
            columns.append("sentiment_LM")
        if "Vader" in param["dicts"]:
            columns.append("sentiment_Vader")

        input_dim = len(columns)

        # load Data
        text_df = pq.ParquetDataset("data/Content.parquet", validate_schema=False, filters=[('cc', 'in', ccs), ('source', '=', param["source"])])
        text_df = text_df.read(columns=columns).to_pandas()
        ...
        for i, (test_roll_start_date, test_roll_end_date) in enumerate(windows):
            ...
            #test_env.render_all(path= "./plots/dictRnnApproaches/{}_{}_{}_{}_{}_{}_{}_{}_CV".format(*[str(x).replace("/","-") for x in list(dict(param).values())]) + "{}.png".format(i))
