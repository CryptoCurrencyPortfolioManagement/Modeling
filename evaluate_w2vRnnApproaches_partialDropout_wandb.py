...
class W2VRecurrentPriceClassifier(nn.Module):
    def __init__(self, use_price, num_assets, embed_dim=300, use_attention= True, rnn_layers= 1, linear_layers= 1, dropout_prop= 0.5):
...
class ModelWrapper(BaseEstimator, ClassifierMixin):
    def __init__(self, batch_size, ccs, use_price= True, epochs= 2, learning_rate= 1e-5, use_attention= False, rnn_layers= 1, linear_layers= 1, dropout_prop= 0.5):
        ...
        if len(GPUtil.getAvailable(order='memory', limit=1, maxLoad=0.5, maxMemory=0.5)) > 0:
            ...
            self.device = "cpu"
        #self.device = "cuda"
        self.model = W2VRecurrentPriceClassifier(use_price, num_assets= len(ccs), embed_dim= 299 + len(ccs), use_attention= use_attention, rnn_layers= rnn_layers, linear_layers= linear_layers, dropout_prop= dropout_prop)
    ...
    def fit(self, X, y):
        ...
        mask = (~np.isnan(X_text_new)).sum(axis=tuple(range(1, X_text_new.ndim))) > 0
...
if __name__ == "__main__":
    with wandb.init() as run:
        param = wandb.config
        #param = {"learning_rate": 0.00018217168033522052, "linear_layers": 1, "rnn_layers": 3, "source": "CoinTelegraph", "use_attention": True, "use_price": False, "window_size": 1, "dropout": 0.5, "epochs": 2}
        ...
        for i, (test_roll_start_date, test_roll_end_date) in enumerate(windows):
            ...
            my_model = ModelWrapper(batch_size= 64, ccs= test_env.labels, learning_rate=param["learning_rate"], use_attention= param["use_attention"], rnn_layers= param["rnn_layers"], linear_layers= param["linear_layers"], use_price= param["use_price"], dropout_prop= param["dropout"], epochs= param["epochs"])
