...
class TransformerRecurrentPriceClassifier(nn.Module):
    def __init__(self, use_price, n_tokens, n_head= 12, transformer_layers= 1, embed_dim=300, embed_model_dim= 300, use_attention= False, rnn_layers= 1, linear_layers= 1, dropout= 0.1):
...
class ModelWrapper(BaseEstimator, ClassifierMixin):
    def __init__(self, batch_size, ccs, n_head, transformer_layers, use_price= True, epochs= 2, learning_rate= 1e-5, use_attention= False, rnn_layers= 1, linear_layers= 1, dropout_prop= 0.5):
        ...
        self.model = TransformerRecurrentPriceClassifier(use_price, len(self.tokenizer), n_head, transformer_layers, use_attention= use_attention, rnn_layers= rnn_layers, linear_layers= linear_layers)
...
if __name__ == "__main__":
    with wandb.init() as run:
        param = wandb.config
        #param = {"source": "CoinTelegraph", "window_size": 5, "learning_rate": 0.00001, "use_attention": False, "rnn_layers": 1, "linear_layers": 6, "use_price": False, "n_head": 30, "transformer_layers": 2, "dropout": 0.5}
        ...
        for i, (test_roll_start_date, test_roll_end_date) in enumerate(windows):
            ...
            my_model = ModelWrapper(batch_size= 2, ccs= test_env.labels, n_head = param["n_head"], transformer_layers = param["transformer_layers"], learning_rate=param["learning_rate"], use_attention= param["use_attention"], rnn_layers= param["rnn_layers"], linear_layers= param["linear_layers"], use_price= param["use_price"], dropout_prop= param["dropout"])
