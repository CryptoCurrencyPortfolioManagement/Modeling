...
# implements positional encoding which is needed as part of the transformer structure
...
# model to encode data via self trained transformer, embeddings of the transformer are then combined via GRUs
class TransformerRecurrentPriceClassifier(nn.Module):
    def __init__(self, use_price, n_tokens, number_assets=10, n_head= 12, transformer_layers= 1, embed_dim=300, portfolio_dim= 32, embed_model_dim= 300, use_attention= False, rnn_layers= 1, linear_layers= 1, portfolio_layers= 1, dropout= 0.1):
        super(TransformerRecurrentPriceClassifier, self).__init__()
        ...
        self.embedding = nn.Embedding(self.n_tokens, embed_model_dim)
        self.pe = PositionalEncoding(embed_model_dim, max_len=d_model, batch_first= True)
        enc_layer = nn.TransformerEncoderLayer(embed_model_dim, n_head, hidden_size, self.dropout, batch_first= True)
        self.encoder = nn.TransformerEncoder(enc_layer, num_layers=self.transformer_layers)

        self.embed_gru = nn.GRU(embed_model_dim, embed_dim, self.rnn_layers, batch_first= True)
        ...
        if self.use_attention:
            self.embed_attention = Attention(embed_dim, embed_dim, embed_dim, batch_first= True)
    ...
    def forward(self, text, price= None):
        ...
        for lag_ind in range(text.shape[1]):
            ...
            msg_temp = self.encoder(msg_temp_emb, src_key_padding_mask= msg_temp[:, 2, :])
            msg_temp = msg_temp[:, 0, :]
            msg_temp = torch.reshape(msg_temp, (*old_shape[:2], -1))
            outputs, hidden = self.embed_gru(msg_temp)
...
class ModelWrapper(BaseEstimator, ClassifierMixin):
    def __init__(self, batch_size, ccs, n_head, transformer_layers, use_price= True, epochs= 2, learning_rate= 1e-5, use_attention= False, rnn_layers= 1, linear_layers= 1, portfolio_layers= 1, portfolio_dim=32, dropout_prop= 0.5):
        ...
        self.model = TransformerRecurrentPriceClassifier(use_price, len(self.tokenizer), number_assets=len(ccs), n_head= n_head, transformer_layers= transformer_layers, use_attention= use_attention, rnn_layers= rnn_layers, linear_layers= linear_layers, portfolio_layers= portfolio_layers, portfolio_dim= portfolio_dim)
...
if __name__ == "__main__":
    with wandb.init() as run:
        param = wandb.config
        #param = {"source": "CoinTelegraph", "window_size": 2, "learning_rate": 0.00001, "use_attention": True, "rnn_layers": 1, "linear_layers": 1, "use_price": True, "n_head": 12, "transformer_layers": 1, "dropout": 0.5, "portfolio_layers": 1, "portfolio_dim": 32}
        ...
        for i, (test_roll_start_date, test_roll_end_date) in enumerate(windows):
            ...
            my_model = ModelWrapper(batch_size= 2, ccs= test_env.labels, n_head = param["n_head"], transformer_layers = param["transformer_layers"], learning_rate=param["learning_rate"], use_attention= param["use_attention"], rnn_layers= param["rnn_layers"], linear_layers= param["linear_layers"], portfolio_layers= param["portfolio_layers"], portfolio_dim= param["portfolio_dim"], use_price= param["use_price"], dropout_prop= param["dropout"])
