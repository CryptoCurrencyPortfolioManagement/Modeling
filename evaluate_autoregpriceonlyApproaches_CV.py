...
class autoReg_agent:
    ...
    def choose_action(self, obs):
        ...
        if self.shared_model == "shared":
            ...
            X_cc = self.enc.transform(X_cc.reshape(-1, 1)).toarray()
            X_temp = np.split(obs, obs.shape[1]/self.number_assets, axis= 1)
            X_temp = np.column_stack([X_1_temp.flatten()for X_1_temp in X_temp])
            X_temp[np.isnan(X_temp)] = 0
...
for param in tqdm(param_grid):
    ...
    for i, (test_roll_start_date, test_roll_end_date) in enumerate(windows):
        ...
        agent = autoReg_agent(test_env.stock_dim, use_price= True, shared_model= param["shared_model"])
        ...
        #test_env.render_all(path= "./plots/autoregpriceonlyApproaches/{}_{}_CV".format(*list(param.values()))+"{}.png".format(i))
        pd.DataFrame(test_env.history).to_excel("./results/autoregpriceonlyApproaches/{}_{}_CV".format(*list(param.values())) + "{}.xlsx".format(i), index=False)
...
results_df.to_excel("./results/Overview_autoregpriceonlyApproaches.xlsx", index= False)