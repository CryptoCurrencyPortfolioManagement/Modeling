...
class BertRecurrentPriceClassifier(nn.Module):
    def __init__(self, use_price, embed_model, embed_dim=300, embed_model_dim= 768, use_attention= False, rnn_layers= 1, linear_layers= 1):
    ...
    def forward(self, text, price= None):
        if self.embed_model is not None:
            ...
            for lag_ind in range(text.shape[1]):
                ...
                # create bert hidden states within the model itself
...
class ModelWrapper(BaseEstimator, ClassifierMixin):
    def __init__(self, batch_size, ccs, bert_model, use_price= True, epochs= 2, learning_rate= 1e-5, use_attention= False, rnn_layers= 1, linear_layers= 1, dropout_prop= 0.5, device= None):
        if not device:
        ...
        self.model = BertRecurrentPriceClassifier(use_price, embed_model= BertModel.from_pretrained(bert_model), use_attention= use_attention, rnn_layers= rnn_layers, linear_layers= linear_layers)
...
if __name__ == "__main__":
    with wandb.init() as run:
        param = wandb.config
        #param = {"learning_rate": 1e-06, "linear_layers": 3, "model": "bert-base-uncased", "rnn_layers": 1, "source": "CoinTelegraph", "use_attention": False, "use_price": True, "window_size": 2, "dropout": 0.5, "epochs": 3}
        ...
        for i, (test_roll_start_date, test_roll_end_date) in enumerate(windows):
            ...
            my_model = ModelWrapper(batch_size= 2, ccs= test_env.labels, bert_model= param["model"], learning_rate=param["learning_rate"], use_attention= param["use_attention"], rnn_layers= param["rnn_layers"], linear_layers= param["linear_layers"], use_price= param["use_price"], dropout_prop= param["dropout"], epochs= param["epochs"], device= device)
