...
class RecurrentPriceClassifier(nn.Module):
    def __init__(self, use_price, embed_dim=300, embed_model_dim= 768, use_attention= False, rnn_layers= 1, linear_layers= 1):
...
class ModelWrapper(BaseEstimator, ClassifierMixin):
    def __init__(self, batch_size, ccs, bert_model, use_price= True, epochs= 2, learning_rate= 1e-5, use_attention= False, rnn_layers= 1, linear_layers= 1, dropout_prop= 0.5):
        ...
        self.model = RecurrentPriceClassifier(use_price, use_attention= use_attention, rnn_layers= rnn_layers, linear_layers= linear_layers)
        ...
        for epoch in range(self.epochs):
            ...
            if self.use_price:
                for X_text_batch, X_price_batch, y_batch in tqdm(dataloader, desc="Epoch: {}".format(epoch + 1)):
                    ...
                    with torch.no_grad():
                        lag_temp = []
                        for lag_ind in range(X_text_batch.shape[1]):
                        ...
                        X_text_batch = torch.stack(lag_temp, dim=1)
    ...
    def predict(self, X):
        ...
        if self.use_price:
            ...
            with torch.no_grad():
                lag_temp = []
                for lag_ind in range(X_text_new.shape[1]):
                    ...
                    msg_temp = torch.reshape(msg_temp, (*old_shape[:2], -1))
                    lag_temp.append(msg_temp)
                X_text = torch.stack(lag_temp, dim=1)

                X_price = X_price.to(self.device, dtype=torch.float)
                preds = self.model(X_text, X_price)
...
if __name__ == "__main__":
    with wandb.init() as run:
        param = wandb.config
        #param = {"dropout": 0.5, "learning_rate": 1e-06, "linear_layers": 1, "model": "yiyanghkust/finbert-tone", "rnn_layers": 1, "source": "CoinTelegraph", "use_attention": True, "use_price": False, "window_size": 1, "epochs": 2 }
        ...
        for i, (test_roll_start_date, test_roll_end_date) in enumerate(windows):
            ...
            my_model = ModelWrapper(batch_size=10, ccs=test_env.labels, bert_model=param["model"], use_price= param["use_price"], learning_rate=param["learning_rate"],use_attention=param["use_attention"], rnn_layers=param["rnn_layers"],linear_layers=param["linear_layers"], dropout_prop= param["dropout"], epochs= param["epochs"])
