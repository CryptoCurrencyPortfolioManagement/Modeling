...
# create bert model and combine hidden states via gru layers
class BertRecurrentPriceClassifier(nn.Module):
    def __init__(self, use_price, embed_model, number_assets, embed_dim=300, portfolio_dim= 32, embed_model_dim= 768, use_attention= False, rnn_layers= 1, linear_layers= 1, portfolio_layers= 1):
        super(BertRecurrentPriceClassifier, self).__init__()
    ...
    def resize_token_embeddings(self, length):
        self.embed_model.resize_token_embeddings(length)

# setup training and prediction routines
class ModelWrapper(BaseEstimator, ClassifierMixin):
    def __init__(self, batch_size, ccs, bert_model, use_price= True, epochs= 2, learning_rate= 1e-5, use_attention= False, rnn_layers= 1, linear_layers= 1, portfolio_layers= 1, portfolio_dim=32, dropout_prop= 0.5, device= None):
        if not device:
        ...
        self.model = BertRecurrentPriceClassifier(use_price, number_assets= len(ccs), embed_model= BertModel.from_pretrained(bert_model), use_attention= use_attention, rnn_layers= rnn_layers, linear_layers= linear_layers , portfolio_layers= portfolio_layers, portfolio_dim= portfolio_dim)
    ...
    def fit(self, X, y):
        ...
        # rearrange data structure and convert inputs to bert-style inputs
        X_text_new = []
    ...
    # helper function to create bert-style input
    def _create_bert_input(self, data, cc, dropout= 0):
        if np.sum((~np.isnan(data)).astype(int)) == 0:
            ...
            # replace cc indicator with word "token" or appropriate cc based on partial dropout settings
...
class NN_agent():
    ...
    def choose_action(self, obs):
        # apply data transformation to get data into the correct shape for the model
...
if __name__ == "__main__":
    with wandb.init() as run:
        param = wandb.config
        #param = {"learning_rate": 1e-06, "linear_layers": 1, "model": "bert-base-uncased", "rnn_layers": 1, "source": "CoinTelegraph", "use_attention": True, "use_price": True, "window_size": 1, "dropout": 0.5, "epochs": 1, "portfolio_layers": 1, "portfolio_dim": 32}
        ...
        for i, (test_roll_start_date, test_roll_end_date) in enumerate(windows):
            ...
            my_model = ModelWrapper(batch_size= 1, ccs= test_env.labels, bert_model= param["model"], learning_rate=param["learning_rate"], use_attention= param["use_attention"], rnn_layers= param["rnn_layers"], linear_layers= param["linear_layers"], portfolio_layers= param["portfolio_layers"], portfolio_dim= param["portfolio_dim"], use_price= param["use_price"], dropout_prop= param["dropout"], epochs= param["epochs"], device= device)
